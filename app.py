...
with st.sidebar:
    ...
    # === 新機能：使えるAIを自動で取得してリスト化する ===
    selected_model = "gemini-1.5-flash"
    if api_key:
        try:
            genai.configure(api_key=api_key.strip())
            # APIキーを使って、現在利用可能なモデルの一覧を取得
            ...
            if available_models:
                st.markdown("---")
                st.write("🧠 AIモデル（エラーが出たらここを変更！）")
                
                # 安定している「1.5-pro」系があればそれを初期設定にする
                default_idx = 0
                ...
                selected_model = st.selectbox("現在使用中の脳みそ", available_models, index=default_idx)
        except Exception:
            pass
            ...
            with st.spinner("音声を文字に変換しています..."):
                try:
                    ...
                    if "429" in str(e):
                        ...
                        st.error(f"⚠️ 無料枠の休憩タイムです。あと {wait_t}秒 ほど経ってからもう一度送信してください☕")
                    elif "400" in str(e) or "not supported" in str(e).lower():
                        st.error(f"⚠️ このモデルは音声入力に対応していません。左側のメニューから別のAIモデルに変更してください。")
                    else:
