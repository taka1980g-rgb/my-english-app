...
if "chat_session" in st.session_state:
    ...
    if is_practice:
        ...
        # ★変更点：お助けツールをチェックボックス（開閉スイッチ）の中に収納しました
        show_tools = st.checkbox("🛠️ お助けツールを開く（日本語訳・辞書・ギブアップなど）")

        if show_tools:
            st.write("※これらを使っても会話は先に進みません（ギブアップを除く）")
